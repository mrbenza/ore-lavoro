...
function getMonthlyWorkData(sessionToken, year, month) {
  ...
  try {
    ...
    console.log('=== getMonthlyWorkData DEBUG ===');
    console.log('sessionToken ricevuto:', sessionToken);
    console.log('year ricevuto:', year, 'tipo:', typeof year);
    console.log('month ricevuto:', month, 'tipo:', typeof month);
    ...
    console.log('lastRow nel foglio:', lastRow);
    ...
    var rowsForThisMonth = 0;
    
    console.log('Processando ' + workData.length + ' righe di dati');
    ...
    for (var i = 0; i < workData.length; i++) {
      ...
      if (!dateCell || ore === '' || ore === null || ore === undefined) {
        console.log('Riga ' + (i+5) + ' saltata: data o ore vuote');
        continue;
      }
      ...
      if (isNaN(workDate.getTime())) {
        console.log('Riga ' + (i+5) + ' saltata: data non valida');
        continue;
      }
      
      console.log('Riga ' + (i+5) + ': data=' + workDate.toDateString() + ', anno=' + workDate.getFullYear() + ', mese=' + (workDate.getMonth() + 1));
      
      if (workDate.getFullYear() === year && workDate.getMonth() === month - 1) {
        rowsForThisMonth++;
        var day = workDate.getDate();
        
        console.log('Match trovato per giorno ' + day + ' con ' + ore + ' ore');
    ...
    console.log('Totale righe processate:', totalRowsProcessed);
    console.log('Righe per questo mese:', rowsForThisMonth);
    console.log('Giorni con dati:', Object.keys(monthlyData).length);
    console.log('monthlyData finale:', monthlyData);
